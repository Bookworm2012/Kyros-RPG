...
Banned = ["monty", "cj", "emiliano", "chase", "mclean"]
...
", a tavern (t), and an adventurers guild branch (a). ")
  ...
  if Location == "money":
    ...
    # Asking if you want to begin fighting the slime, or use an item or retreat
      if  Choice == "a":
        YourTotalDmgDealt, SlimeStats["SlimeHealth"] = PerformAttack(
                                                        AtkPwr,
...
#lines 70 and 72 say how much damage you did to the slime and how much hp it has left
        ...
        if SlimeStats["SlimeHealth"] <= 0: # Victory condition
          ...
          break
        (MonsterTotalDmgDealt, Health) = PerformAttack(
  ...
  BuffEndTime) = InventoryMenu(Idx, Choice, Inventory,
    ...
    if len(Inventory) != 0:
      ...
      Gold, Inventory, ShopStock, TextMode = ShopElya(Gold, Inventory, Choice, Idx,
    ...
    if "Mysterious Letter" in Inventory:
      ...
      if str(SecretCode) == CodeInput:
        ...
        Gold, Inventory = BlacksmithShopElya(Gold, Inventory, Choice)

  elif Location == "t":
    (Gold, AtkPwr, Defense, BuffActive, BuffEndTime, Health,
    MaxHealth, HealthRegenMultiplier, Mana, MaxMana, ManaRegenMultiplier,
    Inventory) = TavernElya(Gold, Choice, Inventory, MaxHealth, Health,
